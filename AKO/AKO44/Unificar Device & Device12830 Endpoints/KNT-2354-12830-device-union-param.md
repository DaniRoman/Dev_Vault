...
`ControllerAbstract.get` ya implementa `_checkPrivilege` + `_checkRestrictions` (filtro de compañía vía `model._company.equals(context.company._id)`). A primera vista parece la opción correcta, pero **no se puede usar** en estos 4 métodos por 3 razones técnicas:

1. **Devuelve un POJO, no un `DeviceModel` de Mongoose.** `get()` termina llamando a `_filterItem` → `item.toJSON()` → `objectHelper.filterByPaths(...)`. El resultado pierde:
   - Métodos de instancia (`device.getUTCOffset()` — usado en `getActivity` línea 2426).
   - Virtuals y getters.
   - El `_id` como `ObjectId` (queda como string).

   `getActivity` necesita `device.getUTCOffset()` → no puede recibir un POJO.

2. **`_filterItem` recorta campos según los permisos del usuario.** Aplica `app.getPaths(modelName, "available", context, item)` y deja solo lo que el user tiene permiso de ver. Para flujos internos que pasan el `device` a `Ad2Metrics._getExportMetric`, al 12830, o al cálculo de fechas en `getMetrics` (que lee `device.lastStatus.lastMsgTimestamp`), necesitamos el device completo — no la vista filtrada del usuario.
